...
public class MaShOAinitRef extends operator{
    ...
    public MaShOAinitRef(MaShOADoubleSolutionSet s){
        this.s=s;
    ...
    public  MaShOADoubleSolutionSet execute(List<ReferencePoint<MaShOADoubleSolution>> ref){
        ...
        int rankingIndex=1;//表示第几层,因为是从帕累托等级为1开始的，所以，你懂
        ...
        while (s.size() < maxsize)
        {
            ...
            MaShOADoubleSolution chosen = SelectClusterMember(referencePoints.get(min_rp));
    ...
    MaShOADoubleSolution SelectClusterMember(ReferencePoint<MaShOADoubleSolution> rp)
    {
        MaShOADoubleSolution chosen = null;
    ...
    int FindNicheReferencePoint()
    {
        ...
        for (ReferencePoint<MaShOADoubleSolution> referencePoint : this.referencePoints)
    ...
    public void associate(MaShOADoubleSolutionSet population) {
        for (int t = 1; t < front.size(); t++) {
            for (MaShOADoubleSolution s : front.get(t)) {
                int min_rp = -1;
